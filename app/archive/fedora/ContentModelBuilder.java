...
public class ContentModelBuilder {
	...
	void createFedoraXMLForContentModels(Node node) {
		...
		for (Transformer model : models) {
			...
			infoStream.append("<ContentModelPid>" + model.getContentModelPID()
					+ "</ContentModelPid>");
			infoStream.append("<ServiceDefPid>" + model.getServiceDefinitionPID()
					+ "</ServiceDefPid>");
			infoStream.append("<ServiceDepPid>" + model.getServiceDeploymentPID()
					+ "</ServiceDepPid>");
			...
			for (int i = 0; i < names.size(); i++) {
				...
				infoStream
						.append("<serviceLocation>" + locs.get(i) + "</serviceLocation>");
				infoStream.append("</Method>");
			}
			infoStream.append("</Methods>");
		...
		try {
			...
			new AddDatastream(node.getPid(), "HBZCMInfoStream").mimeType("text/xml")
					.formatURI("info:hbz/hbz-system:HBZContentModelInfoStream1.0")
	...
	/**
	 * @param cm A fedora-like content model
	...
	String getDsCompositeModel(Transformer cm) {
		...
		for (int i = 0; i < prescribedDSIds.size(); i++) {
			...
			middle.append("<dsTypeModel ID=\"" + dsid + "\">" + "<form FORMAT_URI=\""
					+ furi + "\" MIME=\"" + mtype + "\"/>" + "</dsTypeModel>");
		}
	...
	String getMethodMap(Transformer cm) {
		...
		for (int i = 0; i < cm.getMethodNames().size(); i++) {
			...
			middle.append("<fmm:Method label=\"" + methodName + "\" operationName=\""
					+ methodName + "\"/>");
		}
	...
	String getMethodMapToWsdl(Transformer cm) {
		...
		for (int i = 0; i < cm.getMethodNames().size(); i++) {
			String methodName = cm.getMethodNames().get(i);
			middle.append("    <fmm:Method operationLabel=\"" + methodName
					+ "\" operationName=\"" + methodName + "\" wsdlMsgName=\""
					+ methodName + "Request\" wsdlMsgOutput=\"dissemResponse\">"
	...
	String getWsdl(Transformer cm) {
		String start =
				"<wsdl:definitions name=\"Undefined\" targetNamespace=\"bmech\""
		...
		for (int i = 0; i < cm.getMethodNames().size(); i++) {
			...
			middle.append("<wsdl:operation name=\"" + methodName + "\">"
					+ "<wsdl:input message=\"this:" + methodName + "Request\"/>"
		...
		middle.append(
				"<wsdl:port binding=\"this:HBZContentModelImpl_http\" name=\"HBZContentModelImpl_port\">"
						+ "<http:address location=\"LOCAL\"/>" + "</wsdl:port>");
		middle.append("</wsdl:service>");
		middle.append(
				"<wsdl:binding name=\"HBZContentModelImpl_http\" type=\"this:HBZContentModelPortType\">"
