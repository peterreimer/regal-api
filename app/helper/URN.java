...
public class URN {
	...
	/**
	 * @param snid sub name id
	 * @param niss a self generated id
	 */
	...
	private String getUrnChecksum(String rawUrn) {
		...
		int q =
				ps / Integer.parseInt(String.valueOf(urnArray[urnArray.length - 1]));
	...
	private char[] urnMap(String myurn) {
		...
		for (int i = 0; i < urnArray.length; i++) {
			...
			strBuf.append(Integer
					.parseInt(mKonkordanz.getProperty(String.valueOf(urnArray[i]))));
